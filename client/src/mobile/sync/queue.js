...
// later delete, etc.).
export async function getDueOperations(db, limit = 50) {
  const rows = await dbAll(db, `
    SELECT * FROM sync_queue WHERE status = 'pending' AND next_attempt_at <= ? ORDER BY created_at ASC
...
export async function markInFlight(db, id) {
  await dbRun(db, `UPDATE sync_queue SET status = 'in_flight' WHERE id = ?`, [id]);
}

export async function markDone(db, id) {
  await dbRun(db, `UPDATE sync_queue SET status = 'done' WHERE id = ?`, [id]);
}
...
export async function markFailed(db, id, errorMessage) {
  const row = await dbGet(db, `SELECT attempts FROM sync_queue WHERE id = ?`, [id]);
  const attempts = (row?.attempts ?? 0) + 1;
  const giveUp = attempts >= MAX_ATTEMPTS;
  const nextAttemptAt = new Date(Date.now() + backoffMs(attempts)).toISOString();

  await dbRun(db, `
    UPDATE sync_queue SET attempts = ?, last_error = ?, status = ?, next_attempt_at = ? WHERE id = ?
  `, [attempts, String(errorMessage).slice(0, 2000), giveUp ? 'failed' : 'pending', nextAttemptAt, id]);
}

export async function hasPendingOrInFlight(db, entityType, entityId) {
...
export async function cancelQueuedFor(db, entityType, entityId) {
  await dbRun(db, `
