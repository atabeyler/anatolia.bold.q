...
export default function AppFooter({ fixed = false }) {
  ...
  return (
    <footer className={`${fixed ? 'fixed bottom-0 left-0 right-0' : 'relative'} z-20 py-2 border-t border-gold/20 bg-navy-light/80 backdrop-blur flex items-center justify-center px-4`}>
      <p className="text-[11px] sm:text-[12px] md:text-[13px] lg:text-xs leading-tight text-gold/60 tracking-wide sm:tracking-wider text-center">
        ...
        {' · '}<span className="text-[11px] sm:text-[11px] md:text-[12px] lg:text-[13px] text-gold/40">{t('classified')}</span>
      </p>
    </footer>
  );
}
