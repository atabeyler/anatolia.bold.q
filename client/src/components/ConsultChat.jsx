...
export default function ConsultChat() {
  ...
  return <div className="flex flex-col bg-navy-light/70 border border-gold/30 rounded-lg overflow-hidden" style={{ height: 'calc(100dvh - 200px)', minHeight: 400 }}>
    ...
    <div className="flex-shrink-0 border-t border-gold/20 p-3 bg-navy/30"><div className="flex gap-2 items-end"><textarea ref={inputRef} value={input} onChange={e => { setInput(e.target.value); autoResize(e); }} onKeyDown={handleKey} placeholder={t('consultPlaceholder')} rows={1} className="flex-1 bg-navy/80 border border-gold/30 rounded-lg px-3 py-2.5 text-gold/90 text-sm font-serif focus:border-gold focus:outline-none resize-none" style={{ minHeight: 42, maxHeight: 128 }} /><VoiceButton mode="input" onTranscript={text => setInput(prev => prev ? prev + ' ' + text : text)} size="sm" /><button onClick={send} disabled={(!input.trim() && aiFiles.length === 0) || loading} aria-label="Gönder" className="btn-gold h-10 w-10 rounded-lg flex items-center justify-center disabled:opacity-40 flex-shrink-0">{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}</button></div></div>
  </div>;
}
