...
test('keeps zero false negatives when minRecall is 1', () => {
  const selected = selectConstrainedThreshold(labels, scores, { minRecall: 1, orientations: ['normal'] });
...
test('raises threshold only when recall constraint permits it', () => {
  const selected = selectConstrainedThreshold(labels, scores, { minRecall: 0.5, orientations: ['normal'] });
