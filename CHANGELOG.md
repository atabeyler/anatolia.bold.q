...
### Added
- In-app update banner on desktop and Android, checked via this server's new `/api/version/latest` (proxying GitHub's release lookup so neither client ever calls GitHub's API directly) — approving downloads the installer/APK and, on desktop, offers to install and restart immediately. Replaces desktop's old silent electron-updater check.
